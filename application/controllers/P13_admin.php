...
class P13_admin extends CI_Controller {
	...
	public function index()
	{
		if ($this->session->userdata('isLogin')==TRUE) {
	        $config['base_url'] = site_url('P13_admin/index');
	        $config['total_rows'] = $this->db->count_all('p13');
	        $config['per_page'] = 5;
	        ...
	        $data['P13'] = $this->m_data->get_data_list('p13', $config["per_page"], $data['page']);           
	 
	        $data['pagination'] = $this->pagination->create_links();
			$data['menu_permasalahan'] = $this->m_data->getSemua('permasalahan');
			$this->template->load('admin/index','admin/P13_admin',$data);
	...
	public function create_P13(){
		if ($this->session->userdata('isLogin')==TRUE) {
			...
			$data['menu_permasalahan'] = $this->m_data->getSemua('permasalahan');
			$this->template->load('admin/index','admin/create_P13',$data);
			//$this->template->load('admin/index','admin/footer');
		}else{
			redirect('login');
		}
	...
	public function proses(){
		if ($this->session->userdata('isLogin')==TRUE) {
			...
			$id_permasalahan = $id;
			$data['permasalahan_select_id'] = $this->m_data->cari_permasalahan_id($id_permasalahan);
			$data['konten_select_id'] = $this->m_data->cari_pertanyaan_P13($id_permasalahan);
	...
	public function edit($key_id){
		if ($this->session->userdata('isLogin')==TRUE) {
			...
			$data['P13_id'] = $this->m_data->getN('p13',$variabel);
			$data['menu_permasalahan'] = $this->m_data->getSemua('permasalahan');
			$this->template->load('admin/index','admin/update_P13',$data);
			//$this->template->load('admin/index','admin/footer');
		}else{
			redirect('login');
		}
