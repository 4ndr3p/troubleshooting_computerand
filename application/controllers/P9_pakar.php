...
class P9_pakar extends CI_Controller {
	...
	public function index()
	{
		if ($this->session->userdata('isLogin')==TRUE) {
	        $config['base_url'] = site_url('P9_pakar/index');
	        $config['total_rows'] = $this->db->count_all('p9');
	        $config['per_page'] = 5;
	        ...
	        $data['P9'] = $this->m_data->get_data_list('p9', $config["per_page"], $data['page']);           
	 
	        $data['pagination'] = $this->pagination->create_links();
			$data['menu_permasalahan'] = $this->m_data->getSemua('permasalahan');
			$this->template->load('pakar/index','pakar/P9_pakar',$data);
	...
	public function create_P9(){
		if ($this->session->userdata('isLogin')==TRUE) {
			...
			$data['menu_permasalahan'] = $this->m_data->getSemua('permasalahan');
			$this->template->load('pakar/index','pakar/create_P9',$data);
			//$this->template->load('pakar/index','pakar/footer');
		}else{
			redirect('login');
		}
	...
	public function proses(){
		if ($this->session->userdata('isLogin')==TRUE) {
			...
			$id_permasalahan = $id;
			$data['permasalahan_select_id'] = $this->m_data->cari_permasalahan_id($id_permasalahan);
			$data['konten_select_id'] = $this->m_data->cari_pertanyaan_P9($id_permasalahan);
	...
	public function edit($key_id){
		if ($this->session->userdata('isLogin')==TRUE) {
			...
			$data['P9_id'] = $this->m_data->getN('p9',$variabel);
			$data['menu_permasalahan'] = $this->m_data->getSemua('permasalahan');
			$this->template->load('pakar/index','pakar/update_P9',$data);
			//$this->template->load('pakar/index','pakar/footer');
		}else{
			redirect('login');
		}
