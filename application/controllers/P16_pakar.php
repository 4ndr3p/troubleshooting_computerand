...
class P16_pakar extends CI_Controller {
	...
	public function index()
	{
		if ($this->session->userdata('isLogin')==TRUE) {
	        $config['base_url'] = site_url('P16_pakar/index');
	        $config['total_rows'] = $this->db->count_all('p16');
	        $config['per_page'] = 5;
	        ...
	        $data['P16'] = $this->m_data->get_data_list('p16', $config["per_page"], $data['page']);           
	 
	        $data['pagination'] = $this->pagination->create_links();
			$data['menu_permasalahan'] = $this->m_data->getSemua('permasalahan');
			$this->template->load('pakar/index','pakar/P16_pakar',$data);
	...
	public function create_P16(){
		if ($this->session->userdata('isLogin')==TRUE) {
			...
			$data['menu_permasalahan'] = $this->m_data->getSemua('permasalahan');
			$this->template->load('pakar/index','pakar/create_P16',$data);
			//$this->template->load('pakar/index','pakar/footer');
		}else{
			redirect('login');
		}
	...
	public function proses(){
		if ($this->session->userdata('isLogin')==TRUE) {
			...
			$id_permasalahan = $id;
			$data['permasalahan_select_id'] = $this->m_data->cari_permasalahan_id($id_permasalahan);
			$data['konten_select_id'] = $this->m_data->cari_pertanyaan_P16($id_permasalahan);
	...
	public function edit($key_id){
		if ($this->session->userdata('isLogin')==TRUE) {
			...
			$data['P16_id'] = $this->m_data->getN('p16',$variabel);
			$data['menu_permasalahan'] = $this->m_data->getSemua('permasalahan');
			$this->template->load('pakar/index','pakar/update_P16',$data);
			//$this->template->load('pakar/index','pakar/footer');
		}else{
			redirect('login');
		}
